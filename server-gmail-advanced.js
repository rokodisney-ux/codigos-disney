...
app.get('/api/codigos/:email', async (req, res) => {
    try {
        ...
        // Intentar buscar en Gmail con configuración avanzada
        try {
...
// Función para buscar código REAL en Gmail con configuración avanzada
async function buscarCodigoRealEnGmail(email) {
    return new Promise((resolve, reject) => {
        const Imap = require('imap');
        
        // Configuración avanzada de IMAP
        const imap = new Imap({
        ...
        imap.once('ready', () => {
            console.log('✅ Conectado a Gmail con configuración avanzada');
            
            // Buscar correos de las últimas 4 horas
            const fechaLimite = new Date(Date.now() - 4 * 60 * 60 * 1000);
...
// Iniciar servidor
app.listen(PORT, () => {
    console.log(`🚀 Servidor GMAIL ADVANCED iniciado en http://localhost:${PORT}`);
    console.log(`📊 API disponible en http://localhost:${PORT}/api`);
    console.log(`🌐 Frontend disponible en http://localhost:${PORT}`);
    console.log(`📧 Modo AVANZADO: Configuración mejorada para Gmail`);
