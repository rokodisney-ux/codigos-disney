...
app.get('/api/codigos/:email', async (req, res) => {
    try {
        ...
        // Intentar buscar con IMAP directo (sin guardar en base de datos)
        try {
        ...
        // Si no se encontró en Gmail, buscar en base de datos como fallback
        console.log('🔍 Buscando en base de datos como fallback...');
...
// Función para buscar código REAL en Gmail (sin guardar en base de datos)
async function buscarCodigoRealEnGmail(email) {
    return new Promise((resolve, reject) => {
        ...
        imap.once('ready', () => {
            console.log('✅ Conectado a Gmail para búsqueda REAL');
                        ...
                        try {
                            ...
                            if (codigosValidos.length > 0) {
                                ...
                                resolve({
                                    ...
                                    texto: texto.substring(0, 200) + '...' // Primeros 200 caracteres para debug
                                });
                            } else {
...
app.listen(PORT, () => {
    ...
    console.log(`📧 Modo DINÁMICO: Busca códigos REALES en Gmail`);
    console.log(`🔍 Cada consulta busca el código más reciente que llegó`);
    console.log(`🗄️ Sin base de datos predeterminada`);
    console.log(`🔧 Solo muestra códigos que realmente han llegado`);
});
