...
app.get('/api/codigos/:email', async (req, res) => {
    try {
        ...
        // Buscar directamente en Gmail con IMAP
        const resultado = await buscarCodigoReal(email);
...
async function buscarCodigoReal(email) {
                ...
                fetch.on('message', (msg, seqno) => {
                    msg.on('body', async (stream, info) => {
                        try {
                            ...
                            console.log(`📧 Procesando correo de: ${parsed.from?.value?.[0]?.address || 'Desconocido'}`);
...
// Iniciar servidor
app.listen(PORT, () => {
    console.log(`🚀 Servidor REAL IMAP iniciado en http://localhost:${PORT}`);
    console.log(`📊 API disponible en http://localhost:${PORT}/api`);
    console.log(`🌐 Frontend disponible en http://localhost:${PORT}`);
    console.log(`📧 Modo REAL: Busca códigos REALES en Gmail`);
    console.log(`🔧 Extrayendo códigos de 6 dígitos de correos reales`);
});
